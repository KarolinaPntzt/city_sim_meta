...
--Rotate the produced raster 180 degrees:
UPDATE point_raster
SET facade_raster = ST_SetUpperLeft(
	ST_SetScale(
		facade_raster,
		-ST_ScaleX(facade_raster),
		ST_ScaleY(facade_raster)
	),
	ST_UpperLeftX(facade_raster) + (ST_Width(facade_raster) * ST_ScaleX(facade_raster)),
	ST_UpperLeftY(facade_raster)
);
...
SELECT 'data:image/png;base64,' || encode(ST_AsPNG(st_colormap), 'base64')
FROM (
SELECT ST_Colormap(
	rast1,
	1,
