...
This SQL-script is dedicated to linking Noise simulation output 
...
SURFACE_GEOMETRY ID to the Geom_Noise table. This is also needed from
a FAIR perspective.

*/

--Link Noise simulation output to 3D city model geometry (WallSurface LOD2):
WITH temp_update_Noise_table(geom_id) AS(

SELECT t1.geom_id, t1.value, t1.geom, t2.GMLID, t2.ID,
	ST_3DDistance(t1.geom, t2.GEOMETRY) AS distance
	FROM sim_meta.geom_Noise AS t1
	--WHERE t1.simulationID = 'Lden_malmo_bellevue_DpXXXXX_20240630_v2'
...
SET cityObjectIdentifier = temp_update_Noise_table.gmlid,
surfaceGeometryID = temp_update_Noise_table.ID
FROM temp_update_Noise_table
WHERE sim_meta.Geom_Noise.simulationID = 'Lden_malmo_bellevue_DpXXXXX_20240630_v2'
AND sim_meta.Geom_Noise.geom_id = temp_update_Noise_table.geom_id
