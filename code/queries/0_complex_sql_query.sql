/*
	...
	semantic information with outputs for more than one simulations 
	in urban planning.

	This particular example identifies buildings whose use is 
	residential	and have simulation outputs for obstruction angle 
	exceeding the limit	(>30 degrees) and Lden noise from road traffic 
	exceeding 53 dB(A).

	Buildings with these characteristics are highlighted using a red X3D 
	color in Surface_Data linked to a specific Apperance theme for city
...
--theme for residential buildings not conforming to daylight/noise 
--recommendations.
/*
...
--X3D colors building parts (GroundSurface, WallSurface & RoofSurface)
--corresponding to buildings that are residential and fail to conform
--to recommended daylight access (OA metric) and noise level (Lden) 
--threshold values.
...
--ressidential and do not conform to the sunlight and 
--noise level thresholds to red in the TEXTUREPARAM table.
/*
--Get all GMLIDs of windows whose OA>=30
WITH oa_gmlids AS (

	SELECT sim_meta.geom_oa.cityobjectidentifier AS gmlid_oa
...
noise_gmlids AS (
	SELECT sim_meta.geom_noise.cityobjectidentifier AS gmlid_noise
...
--that do not comply to the sunlight and noise requirements.
problematic_b_ids AS (
	SELECT DISTINCT	b.ID AS building_id

	FROM CITYOBJECT CO INNER JOIN OPENING_TO_THEM_SURFACE OTS
	ON CO.ID = OTS.OPENING_ID INNER JOIN THEMATIC_SURFACE TS
	ON OTS.THEMATIC_SURFACE_ID = TS.ID INNER JOIN BUILDING b
